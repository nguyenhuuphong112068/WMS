...
class ChemSupplierController extends Controller
{
    ...
    private const LABEL = 'nhà cung cấp';

    public function index()
    {
        $datas = DB::table(self::TABLE)->orderBy('name', 'asc')->get();

        session()->put(['title' => 'DỮ LIỆU GỐC - NHÀ CUNG CẤP']);

        return view('pages.materData.ChemSupplier.list', ['datas' => $datas]);
    }
    ...
    private function payload(Request $request): array
    {
        return [
            ...
            'tax_no' => $this->nullable($request->tax_no),
            'note' => $this->nullable($request->note),
        ];
    }
    ...
    private function messages(): array
    {
        return [
            ...
            'tax_no.max' => 'Mã số thuế tối đa 100 ký tự.',
