...
{
    private const TABLE = 'packaging_specifications';
    private const LABEL = 'quy cách đóng gói';

    public function index()
    {
        $datas = DB::table(self::TABLE)->orderBy('name', 'asc')->get();

        session()->put(['title' => 'DỮ LIỆU GỐC - QUY CÁCH ĐÓNG GÓI']);

        return view('pages.materData.PackagingSpecification.list', ['datas' => $datas]);
    }
    ...
    {
        return [
            'name.required' => 'Vui lòng nhập tên quy cách đóng gói.',
            'name.max' => 'Tên quy cách đóng gói tối đa 255 ký tự.',
            'name.unique' => 'Quy cách đóng gói này đã tồn tại.',
        ];
    }
}
