                                    ...
                                    <td class="text-center">
                                        ...
                                        @if ($row->type === 'transfer')
                                            <div class="md-sub mt-1">
                                                <i class="fas fa-arrow-right-long mr-1"></i>{{ $row->to_department_short ?: $row->to_department_name ?: '—' }}
                                            </div>
                                            @if ($row->received_import_id)
                                                <span class="exp-received" title="Phòng nhận đã lấy hàng, mã lô bên đó: {{ $row->received_code }}">
                                                    Đã nhận
                                                </span>
                                            @else
                                                <span class="exp-pending" title="Phòng nhận chưa vào màn hình Nhập Hoá Chất để nhận">
                                                    Chờ nhận
                                                </span>
                                            @endif
                                        @endif
                                            ...
                                            @else
                                                <div class="md-sub mt-1">
                                                    <span class="exp-pending"
                                                        title="Chưa gom vào đợt nào, xem tab Hoá chất chờ huỷ">Chờ huỷ</span>
                                                </div>
                                            @endif
                                        @endif
                                    </td>
                ...
                <div class="exp-pane {{ $activeTab === 'report' ? 'is-active' : '' }}" id="expPaneReport">
                    ...
                    <div class="md-toolbar">
                        <p class="hint">
                            ...
                            <b>{{ $expDate($reportTo) }}</b>, gom theo mã danh mục hoá chất
                            ({{ $report->count() }} mã, {{ $expNum($expReportTimes) }} lượt xuất). Cột <b>Quy Đổi
